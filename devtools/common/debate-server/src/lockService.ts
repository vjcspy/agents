import { EventEmitter } from 'node:events';

type ReleaseFn = () => void;

class Mutex {
  private locked = false;
  private queue: Array<(release: ReleaseFn) => void> = [];

  acquire(): Promise<ReleaseFn> {
    return new Promise((resolve) => {
      const tryAcquire = () => {
        if (!this.locked) {
          this.locked = true;
          resolve(() => this.release());
          return;
        }
        this.queue.push((release) => resolve(release));
      };
      tryAcquire();
    });
  }

  private release(): void {
    if (this.queue.length === 0) {
      this.locked = false;
      return;
    }
    const next = this.queue.shift();
    if (!next) {
      this.locked = false;
      return;
    }
    next(() => this.release());
  }
}

export class LockService {
  private readonly locks = new Map<string, Mutex>();
  private readonly notifiers = new Map<string, EventEmitter>();

  async withLock<T>(debateId: string, fn: () => Promise<T>): Promise<T> {
    const mutex = this.getOrCreateLock(debateId);
    const release = await mutex.acquire();
    try {
      return await fn();
    } finally {
      release();
    }
  }

  notifyNewArgument(debateId: string): void {
    const emitter = this.notifiers.get(debateId);
    if (emitter) emitter.emit('new_argument');
  }

  waitForArgument(debateId: string, timeoutMs: number): Promise<boolean> {
    const emitter = this.getOrCreateNotifier(debateId);
    return new Promise((resolve) => {
      const handler = () => {
        clearTimeout(timer);
        emitter.removeListener('new_argument', handler);
        resolve(true);
      };

      const timer = setTimeout(() => {
        emitter.removeListener('new_argument', handler);
        resolve(false);
      }, timeoutMs);

      emitter.on('new_argument', handler);
    });
  }

  private getOrCreateLock(debateId: string): Mutex {
    const existing = this.locks.get(debateId);
    if (existing) return existing;
    const created = new Mutex();
    this.locks.set(debateId, created);
    return created;
  }

  private getOrCreateNotifier(debateId: string): EventEmitter {
    const existing = this.notifiers.get(debateId);
    if (existing) return existing;
    const emitter = new EventEmitter();
    this.notifiers.set(debateId, emitter);
    return emitter;
  }
}


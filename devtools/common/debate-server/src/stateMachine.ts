import type { ArgumentType, DebateState, Role } from './types.js';
...
export function isActionAllowed(state: DebateState, role: Role, action: Action): boolean {
...
function getAllowedActions(state: DebateState, role: Role): Set<Action> {
  ...
  if (state === 'AWAITING_PROPOSER') {
    if (role === 'proposer') return new Set(['submit_claim', 'submit_appeal', 'submit_resolution']);
    if (role === 'arbitrator') return new Set(['submit_intervention']);
    return new Set();
  }
...
export function calculateNextState(
  ...
  if (argType === 'APPEAL' && argRole === 'proposer') return 'AWAITING_ARBITRATOR';
  if (argType === 'RESOLUTION' && argRole === 'proposer') return 'AWAITING_ARBITRATOR';

  if (argType === 'INTERVENTION' && argRole === 'arbitrator') return 'INTERVENTION_PENDING';

...
export class DebateRequestCompletion extends Command {
  static description = 'Request debate completion (submit RESOLUTION)';
  ...
  async run() {
    const { flags } = await this.parse(DebateRequestCompletion);
    ...
    try {
      ...
      const resp = await client.post(`/debates/${flags['debate-id']}/resolution`, {

...
export class DebateRuling extends Command {
  ...
  async run() {
    ...
    if (flags['client-request-id']) body.client_request_id = flags['client-request-id'];

    try {
      const client = getClient();
      const resp = await client.post(`/debates/${flags['debate-id']}/ruling`, body);

      const data = (resp.data ?? {}) as Record<string, unknown>;
      const filtered = filterWriteResponse(data);
      if (flags['client-request-id']) filtered.client_request_id = flags['client-request-id'];

      output(
        new MCPResponse({
          success: true,
          content: [new MCPContent({ type: ContentType.JSON, data: filtered })],
          metadata: { message: 'Ruling submitted', closed: flags.close ?? false },
        }),
        flags.format,
      );
    } catch (e) {

...
export class DocsDelete extends Command {
  ...
  async run() {
    ...
    if (!flags.confirm) {
      output(errorResponse('INVALID_INPUT', 'Deletion requires confirmation', 'Add --confirm flag to proceed'), flags.format);
      this.exit(4);
    }

    const affected = db.softDeleteDocument(args.document_id);
    if (affected === 0) {
      output(errorResponse('DOC_NOT_FOUND', `Document '${args.document_id}' not found or already deleted`, "Use 'aw docs list' to see available documents"), flags.format);
      this.exit(2);
    }
    ...
    output(
      new MCPResponse({
        success: true,
        content: [new MCPContent({ type: ContentType.TEXT, text: 'Document soft-deleted' })],
        metadata: { document_id: args.document_id, versions_affected: affected },
      }),
      flags.format,
    );
  }

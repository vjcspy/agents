...
export class DocsList extends Command {
  ...
  static flags = {
    ...
    'include-deleted': Flags.boolean({ description: 'Include soft-deleted documents' }),
  ...
  async run() {
    ...
    const [documents, total] = db.listDocuments(flags.limit, flags['include-deleted']);

    output(
      new MCPResponse({
        success: true,
        content: [new MCPContent({ type: ContentType.JSON, data: { documents } })],
        totalCount: total,

...
import { MCPResponse, MCPContent, ContentType, output, errorResponse, readContent } from '@aweave/cli-shared';
import * as db from '../../lib/db';
import { validateFormatNoPlain, parseMetadata } from '../../lib/helpers';

export class DocsSubmit extends Command {
  static description = 'Submit a new version of existing document';

  static args = {
    document_id: Args.string({ required: true, description: 'Document ID to update' }),
  };
  ...
  static flags = {
    ...
    metadata: Flags.string({ default: '{}', description: 'JSON metadata object' }),
  ...
  async run() {
    ...
    if (metaErr) { output(metaErr, flags.format); this.exit(4); }

    try {
      const doc = db.submitVersion(args.document_id, flags.summary, result.content!, meta!);
      if (!doc) {
        output(errorResponse('DOC_NOT_FOUND', `Document '${args.document_id}' not found or deleted`, "Use 'aw docs list' to see available documents"), flags.format);
        this.exit(2);
      }
      ...
      output(
        new MCPResponse({
          ...
          metadata: { message: `Version ${doc.version} submitted successfully` },
        }),
        flags.format,
      );
    } catch (e) {

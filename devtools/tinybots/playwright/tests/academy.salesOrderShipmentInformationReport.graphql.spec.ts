...
test('academy GraphQL salesOrderShipmentInformationReport', async ({ page }) => {
  ...
  await test.step('Execute query and verify GraphQL response', async () => {
    ...
    const graphqlResponsePromise = page.waitForResponse(
      (res) => {
        ...
        return post.includes('salesOrderShipmentInformationReport') && res.status() === 200;
      },
      { timeout: 60_000 },
    );
    ...
    const reportRows = graphqlJson.data?.reports?.allReports?.salesOrderShipmentInformationReport;

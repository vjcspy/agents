...
export class BitbucketComments extends Command {
  static description = 'List all PR comments (auto-fetches all pages)';
  ...
  async run() {
    const { args, flags } = await this.parse(BitbucketComments);
    const client = this.getClient(flags.workspace);
    const response = await client.listPRComments(args.repo, parseInt(args.pr_id, 10), flags.max);

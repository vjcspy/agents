...
def find_python_plugins() -> dict:
    ...
    for domain in DOMAINS:
        ...
        for pkg_dir in cli_dir.iterdir():
            if not pkg_dir.is_dir() or pkg_dir.name == "aw-core":
                continue
            ...
            try:
                ...
                # Check if it has aw.plugins entry point
                entry_points = data.get("project", {}).get("entry-points", {})
                if "aw.plugins" in entry_points:
                    for name, entry in entry_points["aw.plugins"].items():
                        plugins[name] = {
                            "type": "python",
                            "entry": entry,
...
def find_node_plugins() -> dict:
    """Find Node plugins by scanning for aw-plugin.yaml markers."""
    plugins = {}

    for marker in REPO_ROOT.rglob("aw-plugin.yaml"):
        ...
        try:
            ...
            if not name:
                print(f"Warning: aw-plugin.yaml at {marker} missing 'name' field")
                continue

            bin_path = config.get("bin")
            if not bin_path:
                print(f"Warning: aw-plugin.yaml at {marker} missing 'bin' field")
                continue
...
def main():
    ...
    output = REPO_ROOT / "aw-plugins.yaml"
